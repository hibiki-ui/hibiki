...
class ResponsiveGrid(GridContainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(children=children, columns=column_template, gap=gap, style=style, **kwargs)
    ...
    def update_responsive_layout(self, container_width: float):
        ...
        gap_value = (
            16
            if isinstance(self.style.gap, type(None))
            else (self.style.gap.value if hasattr(self.style.gap, "value") else 16)
        )
...
class ZStack(Container):
    ...
    def add_layer(
        self, child: Component, z_index: int = 0, offset_x: float = 0, offset_y: float = 0
    ):
        """添加层级子组件
...
class MasonryContainer(Container):
    ...
    def add_masonry_item(self, child: Component):
        ...
        # 在实际实现中，需要在布局计算后更新
        estimated_height = (
            getattr(child.style, "height", px(100)).value
            if hasattr(child.style, "height") and child.style.height
            else 100
        )
...
class SplitView(Container):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 🔧 直接设置组件的flex属性，不使用包装容器
        # 确保primary和secondary有style属性
        if not hasattr(primary, "style") or primary.style is None:
            primary.style = ComponentStyle()
        if not hasattr(secondary, "style") or secondary.style is None:
        ...
        if primary.style.height is None:
            primary.style.height = percent(100)
        ...
        if secondary.style.height is None:
            secondary.style.height = percent(100)

        # 保存原始引用
        self._original_primary = primary
        self._original_secondary = secondary
    ...
    def _create_splitter(self) -> Component:
        ...
        # 🔧 关键修复：分割线不参与flex空间分配
        splitter_style.flex_grow = 0
        splitter_style.flex_shrink = 0
        splitter_style.background_color = "#cccccc"  # 添加默认背景色便于识别
...
class LayoutPresets:
    ...
    def card_grid(columns: int = 3, gap: int = 16) -> GridContainer:
        ...
        return GridContainer(
            columns=f"repeat({columns}, 1fr)", gap=gap, style=ComponentStyle(padding=px(16))
        )
    ...
    def sidebar_layout(sidebar_width: int = 250) -> SplitView:
        ...
        sidebar = BasicContainer(style=ComponentStyle(width=px(sidebar_width), min_width=px(200)))
        main = BasicContainer()
    ...
    def header_content_layout(header_height: int = 60) -> VStack:
        ...
        header = BasicContainer(style=ComponentStyle(height=px(header_height), width=percent(100)))

        content = BasicContainer(style=ComponentStyle(flex_grow=1))

        return VStack(
            children=[header, content], spacing=0, style=ComponentStyle(height=percent(100))
        )

    @staticmethod
    def masonry_gallery(columns: int = 3) -> MasonryContainer:
        """瀑布流画廊"""
        return MasonryContainer(columns=columns, gap=12, style=ComponentStyle(padding=px(12)))
...
class LayoutAnimator:
    ...
    @staticmethod
    def animate_grid_resize(
        grid: GridContainer, new_columns: Union[str, List[str]], duration: float = 0.3
    ):
        """动画化Grid列变化"""
        # TODO: 实现布局动画
        pass
...
if __name__ == "__main__":
    ...
    grid = GridContainer(columns="repeat(3, 1fr)", rows="100px auto", gap=16)
    ...
    # 测试响应式Grid
    logger.info("\n📱 响应式Grid测试:")
    responsive_grid = ResponsiveGrid(min_column_width=200, max_columns=4, gap=16)
