...
if __name__ == "__main__":
    logger.debug("Hibiki UI v4.0 基础组件测试\n")
    ...
    logger.debug("🧪 基础组件创建测试:")

    # 创建Label
    label = Label("Hello, Hibiki UI v4.0!")
    logger.debug(f"Label创建完成: {label.__class__.__name__}")
    ...
    logger.debug(f"Button创建完成: {button.__class__.__name__}")
    ...
    logger.info(
        f"样式化Label: opacity={styled_label.style.opacity}, scale={styled_label.style.scale}"
    )
    ...
    logger.info(
        f"高级Button: position={advanced_button.style.position}, rotation={advanced_button.style.rotation}°"
    )
