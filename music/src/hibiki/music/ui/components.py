...
from hibiki.ui import (
    UIComponent, CustomView, Container, Label, Button, 
    ComponentStyle, px, Signal, Computed, Effect,
    DrawingUtils
)
...
class MusicProgressBar(UIComponent):
    ...
    def __init__(
        ...
        super().__init__(style or ComponentStyle(
            width=px(300), 
            height=px(20)
        ))
...
class AlbumArtView(UIComponent):
    ...
    def _create_nsview(self):
        ...
        custom_view = CustomView(
            style=self.style,
            on_draw=draw_album_art,
            on_mouse_up=on_click_handler
        )
    ...
    def _create_nsview(self):
        ...
        def draw_volume_slider(context, rect, bounds):
            ...
            if self.orientation == "horizontal":
                ...
                # 滑轨背景
                DrawingUtils.fill_rect(context, 0, track_y, width, track_height, (0.8, 0.8, 0.8, 1.0))
                
                # 音量条
                if not self.is_muted.value:
                    volume_width = width * self.volume.value
                    DrawingUtils.fill_rect(context, 0, track_y, volume_width, track_height, (0.0, 0.5, 1.0, 1.0))
                ...
                # 滑轨背景
                DrawingUtils.fill_rect(context, track_x, 0, track_width, height, (0.8, 0.8, 0.8, 1.0))
                
                # 音量条 (从底部开始)
                if not self.is_muted.value:
                    volume_height = height * self.volume.value
                    volume_y = height - volume_height
                    DrawingUtils.fill_rect(context, track_x, volume_y, track_width, volume_height, (0.0, 0.5, 1.0, 1.0))
...
class SongListItem(UIComponent):
    ...
    def _create_nsview(self):
        ...
        def draw_song_item(context, rect, bounds):
            ...
            DrawingUtils.draw_text(context, self.artist, 30, height/2 + 4, 12, (0.6, 0.6, 0.6, 1.0))
            DrawingUtils.draw_text(context, self.duration, width - 60, height/2 - 6, 12, (0.6, 0.6, 0.6, 1.0))
...
# 便捷函数
def create_music_progress_bar(progress: Signal[float], duration: Signal[float], on_seek: Callable = None):
    """创建音乐进度条的便捷函数"""
